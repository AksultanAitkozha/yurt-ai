...
const About = () => {
  ...
  return (
    <div className="bg-white text-black p-8 md:p-16 flex flex-col md:flex-row space-y-8 md:space-y-0 md:space-x-8">
      <div className="md:w-1/2 space-y-4">
        ...
        <h3 className="text-2xl mb-4" style={{ fontFamily: 'Inter' }}>
        ...
        <Link href = "/Exterior">
        <button className="inline-block bg-black text-white px-6 py-3 text-lg font-bold rounded mt-4 transition duration-300 hover:bg-gray-800">
          Go to the YurtAI app -&gt;
        </button>
        </Link>
      </div>
      <div className="md:w-1/2 relative">
        <img src="header_1.png" alt="Before" className="absolute w-full h-full" style={{ clipPath: `inset(0 ${100 - sliderValue}% 0 0)` }} />
        <img src="yurt_ai.png" alt="After" className="absolute w-full h-full" style={{ clipPath: `inset(0 0 0 ${sliderValue}%)` }} />
        <input type="range" min="0" max="100" value={sliderValue} onChange={(e) => setSliderValue(e.target.value)} className="absolute bottom-0 left-0 w-full" />
      </div>
    </div>
  );
};
